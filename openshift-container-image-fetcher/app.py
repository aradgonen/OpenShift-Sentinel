...
base_url = "/api/images"
openshift_pods_url = "https://api.projocp.cloudlet-dev.com:6443/api/v1/pods"
openshift_replicaset_url = "https://api.projocp.cloudlet-dev.com:6443/apis/apps/v1/namespaces/dummy-namespace/" \
                           "replicasets/{}"
api = Flask(__name__)
...
def get_container_images():
    ...
    pods_details = requests.get(openshift_pods_url, verify=False,  headers=headers)
    ...
    for container_item in pods_dict['items']:
        for container in container_item['spec']['containers']:
            ...
            if image_match:
                ...
                single_container_result['ownerOwnerKind'] = 'NONE'
                single_container_result['ownerOwnerName'] = 'NONE'

                if str.lower(single_container_result['ownerKind']) == "replicaset":
                    replicaset_details = requests.get(openshift_replicaset_url.format(single_container_result['ownerName']),
                                                verify=False, headers=headers)
                    replicaset_dict = replicaset_details.json()
                    single_container_result['ownerOwnerKind'] = replicaset_dict['metadata']['ownerReferences'][0]['kind']
                    single_container_result['ownerOwnerName'] = replicaset_dict['metadata']['ownerReferences'][0]['name']
                    print()
